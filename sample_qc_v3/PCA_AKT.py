...
if __name__ == "__main__":
    ...
    mt_akt = hl.read_matrix_table(f"{temp_dir}/AKT_data/wes.hg38.AKT.mt")
    mt = mt_akt
    # filter mt
   ...
   # )
    ...
    pruned_mt.write(
        f"{tmp_dir}/ddd-elgh-ukbb/AKT_snps_filtered_ldpruned.mt", overwrite=True)
    # run pca
    logger.info("run pca")
    ...
    pca_scores.write(
        f"{tmp_dir}/ddd-elgh-ukbb/AKT_pca_scores.ht", overwrite=True)
    loadings_ht.write(
        f"{tmp_dir}/ddd-elgh-ukbb/AKT_pca_loadings.ht", overwrite=True)
    with open(f"{temp_dir}/ddd-elgh-ukbb/AKT_pca_evals.txt", 'w') as f:
    ...
    ht.write(f"{tmp_dir}/ddd-elgh-ukbb/pc_project_AKT.ht", overwrite=True)
    mt1 = project_mt.annotate_cols(scores=ht[project_mt.s].scores)
    mt1.write(
        f"{tmp_dir}/ddd-elgh-ukbb/project_AKT_projected_scores.mt", overwrite=True)
    ht.export(f"{temp_dir}/ddd-elgh-ukbb/pc_project_AKT.txt.bgz")
