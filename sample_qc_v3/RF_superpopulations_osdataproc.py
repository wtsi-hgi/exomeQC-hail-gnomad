...
if __name__ == "__main__":
    ...
    pca_scores = pca_scores.annotate(
        known_pop=pruned_mt.cols()[pca_scores.s].superpopulation)
    ##########################################################
    # mt = mt.annotate_cols(
    ...
    pca_scores.write(
        f"{tmp_dir}/ddd-elgh-ukbb/pca_scores_after_pruning_superpops.ht", overwrite=True)
    pca_loadings.write(
        f"{tmp_dir}/ddd-elgh-ukbb/pca_loadings_after_pruning_superpops.ht", overwrite=True)
    with open(f"{temp_dir}/ddd-elgh-ukbb/pca_evals_after_pruning_superpops.txt", 'w') as f:
   ...
   # population_assignment_table = assign_population_pcs(
    ...
    pop_ht.write(
        f"{tmp_dir}/ddd-elgh-ukbb/pop_assignments_updated_august2020_superpops.ht", overwrite=True)
    pop_ht.export(
        f"{temp_dir}/ddd-elgh-ukbb/pop_assignments_updated_august2020_superpops.tsv.gz")
