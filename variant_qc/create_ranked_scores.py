...
def create_rf_rank(data_type: str, run_hash: str) -> None:
    ...
    if not hl.hadoop_exists(f'gs://gnomad-tmp/gnomad_{data_type}_rf_{run_hash}.ht/_SUCCESS'):
        ...
        ht.write(
            f'gs://gnomad-tmp/gnomad_{data_type}_rf_{run_hash}.ht', overwrite=True)
...
def create_vqsr_rank_ht(data_type: str):
    ...
    ht = hl.read_table(
        f'gs://gnomad-tmp/gnomad_{data_type}_vqsr_pre_ranking.ht')
...
def get_gnomad_annotations(data_type: str) -> hl.Table:
    ...
    if not hl.utils.hadoop_exists(f'gs://gnomad/variant_qc/temp/gnomad_{data_type}_annotations.ht'):
        ht = get_gnomad_data(data_type).rows()
        call_stats_data = hl.read_table(
            annotations_ht_path(data_type, 'call_stats'))
        freq_data = hl.read_table(
            annotations_ht_path(data_type, 'frequencies'))
        ht = ht.select(
...
def create_cnn_rank_file() -> None:
    ...
    if not hl.utils.hadoop_exists('gs://gnomad/variant_qc/temp/friedman_cnn_scores.ht/_SUCCESS'):
        ...
        ht = hl.import_table(
            'gs://gnomad/variant_qc/temp/friedman_cnn_scores.tsv.bgz', min_partitions=1000, impute=True)
    ...
    ht = ht.annotate(alt_alleles=ht.Alt.split(
...
def create_rf_2_0_2_rank(data_type: str, beta: bool) -> None:
    ...
    if not hl.hadoop_exists(f'gs://gnomad-tmp/gnomad_rf_2_0_2_{data_type}_{str(beta)}_tmp.ht'):
        ht = hl.import_table(get_2_0_2_rf_path(data_type, beta), types={
                             'chrom': hl.tstr}, impute=True, min_partitions=1000)
        if 'chrom' in ht.row:
            ht = ht.transmute(
    ...
    ht.write(score_ranking_path(data_type, 'rf_2.0.2{}'.format(
        '_beta' if beta else '')), overwrite=True)
...
def main(args):
    ...
    def run_data(rank_func: Callable[..., None], rank_func_args: List[Any], score_type: str, data_type: str) -> None:
        ...
        if score_type in ['vqsr', 'cnn', 'rf_2.0.2', 'rf_2.0.2_beta']:
            ...
            rank_file_path = rf_path(
                data_type, 'rf_result', run_hash=args.run_hash)
        ...
        if args.create_binned_file:
            ...
            binned_ht = create_binned_data(
                ht, score_type, data_type, args.n_bins)
            binned_ht.write(score_ranking_path(
                data_type, score_type, binned=True), overwrite=True)
    ...
    for data_type in data_types:
        ...
        if args.run_hash:
            run_data(create_rf_rank, [data_type,
                                      args.run_hash], args.run_hash, data_type)
        ...
        if args.rf_2_0_2:
            run_data(create_rf_2_0_2_rank, [
                     data_type, False], 'rf_2.0.2', data_type)

        if args.rf_2_0_2_beta:
            run_data(create_rf_2_0_2_rank, [
                     data_type, True], 'rf_2.0.2_beta', data_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--genomes', help='Run on genomes. At least one of --exomes or --genomes is required.',
                        action='store_true')
    parser.add_argument(
        '--run_hash', help='Run hash for RF results to be ranked.')
    parser.add_argument(
        '--vqsr', help='When set, creates the VQSR rank file.', action='store_true')
    parser.add_argument(
        '--cnn', help='When set, creates the CNN rank file. Note that there is no CNN file for exomes.', action='store_true')
    parser.add_argument(
        '--rf_2_0_2', help='When set, creates the 2.0.2 RF rank file.', action='store_true')
    parser.add_argument(
        '--rf_2_0_2_beta', help='When set, creates the 2.0.2 RF beta (no medians, QD / max(pAB)) rank file.', action='store_true')
    parser.add_argument(
        '--create_rank_file', help='When set, creates ranking file.', action='store_true')
    parser.add_argument('--run_sanity_checks',
                        help='When set, runs ranking sanity checks.', action='store_true')
    parser.add_argument('--create_binned_file',
                        help='When set, creates binned ranked file.', action='store_true')
    parser.add_argument(
        '--n_bins', help='Number of bins for the binned file (default: 100)', default=100, type=int)
