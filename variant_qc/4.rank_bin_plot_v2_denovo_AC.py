...
def create_binned_data_initial(ht: hl.Table, data: str, data_type: str, n_bins: int) -> hl.Table:
    ...
    return (
        ...
        .aggregate(
            ...
            n_trans_singletons_synonymous=hl.agg.filter((ht.AC[0] < 3) & (ht.consequence == "synonymous_variant") & (
                ht.AC[0] == 1), hl.agg.sum(ht.family_stats.tdt[0].t)),
            ...
            n_untrans_singletons_synonymous=hl.agg.filter((ht.AC[0] < 3) & (ht.consequence == "synonymous_variant") & (
                ht.AC == 1), hl.agg.sum(ht.family_stats.tdt[0].u)),
...
def main(args):
    ...
    ht = hl.read_table(
        f'{temp_dir}/ddd-elgh-ukbb/variant_qc/models/{run_hash}/{run_hash}_rf_result_sanger_cohorts_DENOVO_family_stats_SYNONYMOUS_AC.ht')
    ...
    if args.add_rank:
        ...
        ht_ranked = ht_ranked.checkpoint(
            f'{tmp_dir}/ddd-elgh-ukbb/{run_hash}_rf_result_ranked_denovo_AC.ht', overwrite=True)
    ...
    if args.add_bin:
        ...
        ht = hl.read_table(
            f'{temp_dir}/ddd-elgh-ukbb/variant_qc/models/{run_hash}/{run_hash}_rf_result_ranked_denovo_AC.ht')
        ...
        ht_bins.write(
            f'{tmp_dir}/ddd-elgh-ukbb/{run_hash}_rf_result_ranked_BINS_denovo_AC.ht', overwrite=True)
