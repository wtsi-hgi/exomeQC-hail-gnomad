...
lustre_dir = "file:///lustre/scratch123/wes_jc_ukb_ibd/ruth_training_datasets/variantqc"
resources_dir = "file:///lustre/scratch123/wes_jc_ukb_ibd/ruth_training_datasets/resources/"
######################################
...
def main(args):
    ...
    truthset_ht.write(
        f'{args.output_dir}/truthset_table.ht', overwrite=True)
    # Trio data
    # trio annotation:
    ...
    trio_dataset.checkpoint(
        f'{args.output_dir}/MegaWES_trios_adj.mt', overwrite=True)
    ...
    trio_stats_ht.write(
        f'{args.output_dir}/MegaWES_stats.ht', overwrite=True)
    ...
    mt=mt.checkpoint(f'{args.output_dir}/MegaWESSanger_cohorts_sampleQC_filtered_split.mt', overwrite=True)
    fam = args.trio_fam
    pedigree = hl.Pedigree.read(fam)
    ...
    de_novo_table.write(
        f'{args.output_dir}/MegaWES_denovo_table.ht', overwrite=True)
...
if __name__ == "__main__":
    ...
    input_params.add_argument(
        ...
        default=f'{lustre_dir}/MegaWESSanger_cohorts_sampleQC_filtered_chr19-Y.mt',
        type=str,
    )
    input_params.add_argument(
        "--omni",
        help="Full path of Omni truset hail table GRCh38",
        default=f'{resources_dir}/training_sets/1000G_omni2.5.hg38.ht',
        type=str,
    )
    input_params.add_argument(
        "--mills",
        help="Full path of Mills_1000G_indels truset hail table GRCh38",
        default=f'{resources_dir}/training_sets/Mills_and_1000G_gold_standard.indels.hg38.ht',
        type=str,
    )
    input_params.add_argument(
        "--thousand_genomes",
        help="Full path of 1000 genomes project truset hail table GRCh38",
        default=f'{resources_dir}/training_sets/1000G_phase1.snps.high_confidence.hg38.ht',
        type=str,
    )
    input_params.add_argument(
        "--hapmap",
        help="Full path of Hapmap truset hail table GRCh38",
        default=f'{resources_dir}/training_sets/hapmap_3.3.hg38.ht',
        type=str,
    )
    input_params.add_argument(
        "--trio_fam",
        help="Full path of trio fam file for cohort",
        default=f"{resources_dir}/trios/DDD_trios.fam",
        type=str,
    )
    input_params.add_argument(
        "--priors",
        help="Full path of prior AF for gnomad cohort",
        default=f'{resources_dir}/gnomad_v3-0_AF.ht',
        type=str,
    )
