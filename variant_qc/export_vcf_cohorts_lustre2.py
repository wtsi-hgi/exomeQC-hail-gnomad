...
if __name__ == "__main__":
    ...
    mt = mt.checkpoint(
        f'{lustre_dir}/variant_qc/megaWES_mt_with_stats_w20_June_2021.mt', overwrite=True)
    hl.export_vcf(
        mt, f'{lustre_dir}/variant_qc/megaWES_mt_after_RF_cohort_stats_w20.vcf.bgz', parallel='separate_header')

    mt1 = mt.select_entries()
    mt_fin = mt1.filter_cols(mt1['s'] == 'sample')
    hl.export_vcf(
        mt_fin, f"{lustre_dir}/variant_qc/megaWES_mt_after_RF_stats_for_VEP_w20.vcf.bgz", parallel='separate_header')
