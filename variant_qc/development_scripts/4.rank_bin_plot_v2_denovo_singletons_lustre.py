    ...
    return (
        ...
        .aggregate(
            ...
            n_trans_singletons_synonymous_original=hl.agg.filter((ht.ac_raw < 3) & (ht.consequence == "synonymous_variant") & (
                ht.family_stats.unrelated_qc_callstats.AC[0][1] == 1), hl.agg.sum(ht.family_stats.tdt[0].t)),
            n_untrans_singletons_old=hl.agg.filter((ht.ac_raw < 3) & (
            ...
            n_untrans_singletons_synonymous_original=hl.agg.filter((ht.ac_raw < 3) & (ht.consequence == "synonymous_variant") & (
                ht.family_stats.unrelated_qc_callstats.AC[0][1] == 1), hl.agg.sum(ht.family_stats.tdt[0].u)),
...
def main(args):
    ...
    if args.add_rank:
        ...
        ht_ranked = ht_ranked.checkpoint(
            f'{lustre_dir}/variant_qc/models/{run_hash}_rf_result_ranked_denovo_ddd.ht', overwrite=True)
    ...
    if args.add_bin:
        ...
        ht = hl.read_table(
            f'{lustre_dir}/variant_qc/models/{run_hash}_rf_result_ranked_denovo_ddd.ht')
        ...
        ht_bins.write(
            f'{lustre_dir}/variant_qc/models/{run_hash}_rf_result_ranked_BINS_denovo_ddd.ht', overwrite=True)
